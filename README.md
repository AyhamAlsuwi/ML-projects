...
SimpleRatioTransformer: Engineered ratio features (e.g., sqft_lot vs sqft_lot15 and sqft_living vs sqft_living15) to capture property renovations and expansions over time.

Geographical Clustering: Implemented a KMeans clustering pipeline (n_clusters=37) on lat and long coordinates to group neighborhoods by geographic value.
...
Linear Regression: Severely underperformed. The relationships between variables (like geo-coordinates and price) are highly non-linear.

Random Forest (V1): Established a strong baseline. Far outperformed Linear Regression on RMSE and MAPE.

Iteration 2: Hyperparameter Tuning (V2)

Conducted extensive tuning using RandomizedSearchCV and GridSearchCV on the Random Forest architecture.
...
Refined the KMeans cluster count and ran a deeper randomized search on the Random Forest.

The model effectively learned regional pricing bounds based on the geo-clusters rather than relying on noisy zipcodes.
...
Target Encoding (V4): Attempted replacing geo-clusters with target encoding on the zipcode. This resulted in slightly worse generalization.

XGBoost: Built an XGBoost pipeline and tuned learning rates, gamma, and child weights. Despite extensive tuning, the geo-clustered Random Forest (V3) maintained superior performance metrics.

4. Final Production Metrics

To ensure the model's reliability in production, a 95% Confidence Interval was calculated using Scipy statistical bootstrapping on the unseen test set errors.
